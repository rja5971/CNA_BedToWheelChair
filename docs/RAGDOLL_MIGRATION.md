...
2. Place a `PatientActor`, `BeltActor`, `WheelchairActor`, and `TransferManagerActor` in the level.
...
4. On `WheelchairActor`, assign the CNA wheelchair mesh from `/Game/Project/EventProps/WheelChair` and align its `SeatTarget` and `SeatZone` to the physical chair.
...
The `BeingTransferred` data asset uses the pivot behavior added by the latest Ragdoll implementation: the pelvis position remains fixed while two belt-hand positions drive patient yaw.
